...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $major = mysqli_real_escape_string($link, trim($_POST['major']));
        ...
        $sql = "INSERT INTO education_student_activity_attendance_school_year_2028 (student_id, fullname, year_level, major) 
                VALUES ('$student_id', '$fullname', '$year_level', '$major')";

        if (mysqli_query($link, $sql)) {
            echo '<script>alert("Record added successfully.");</script>';
            echo '<script>window.location.href = "admin_education_student_activity_attendace_school_year_2028.php";</script>';
        } else {
