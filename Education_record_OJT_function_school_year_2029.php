...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $ojt_amount_paid = mysqli_real_escape_string($link, trim($_POST['ojt_amount_paid']));
        ...
        $sql = "INSERT INTO education_student_record_school_year_2029 (student_name, ojt_fee, ojt_amount_paid) 
                VALUES ('$student_name', '$ojt_fee', '$ojt_amount_paid')";

        if (mysqli_query($link, $sql)) {
            echo '<script>alert("Record added successfully.");</script>';
            echo '<script>window.location.href = "admin_cashier_education_department_ojt_fee_school_year_2029.php";</script>';
        } else {
