...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['third_year_student_id'])) {
    // Retrieve and sanitize the id
    $third_year_student_id = mysqli_real_escape_string($link, $_POST['third_year_student_id']);

    // Prepare the DELETE query
    $query = "DELETE FROM third_year_education_first_sem_student_record_school_year_2025 WHERE third_year_student_id = '$third_year_student_id'";

    // Execute the query
    if (mysqli_query($link, $query)) {
        // Redirect to the main page or display a success message
        header('Location: admin_education_third_year_student_grade_year_2025.php'); // Adjust as necessary
        exit;
    } else {
