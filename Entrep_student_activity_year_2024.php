...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $year_level = mysqli_real_escape_string($link, trim($_POST['year_level']));
        ...
        $sql = "INSERT INTO entrep_student_activity_attendance_school_year_2030 (fullname, year_level) 
                VALUES ('$fullname', '$year_level')";

        if (mysqli_query($link, $sql)) {
            echo '<script>alert("Record added successfully.");</script>';
            echo '<script>window.location.href = "admin_entrepreneurship_student_activity_attendace_school_year_2024.php";</script>';
        } else {
