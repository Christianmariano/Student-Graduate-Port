...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['fourth_year_id'])) {
    // Retrieve and sanitize the id
    $fourth_year_id = mysqli_real_escape_string($link, $_POST['fourth_year_id']);

    // Prepare the DELETE query
    $query = "DELETE FROM entrepreneurship_student_record_school_year_2025 WHERE fourth_year_id = '$fourth_year_id'";
    
    // Execute the query
    if (mysqli_query($link, $query)) {
        // Redirect to the main page or display a success message
        header('Location: admin_entrep_fourth_year_student_grade_year_2025.php'); // Adjust as necessary
        exit;
    } else {
