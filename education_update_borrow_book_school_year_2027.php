...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($student_id)) {
        ...
        $currentStudentIdQuery = "SELECT student_id FROM education_student_library_borrow_year_2027 WHERE id = '$id'";
    ...
    if (!empty($student_name)) {
        ...
        $currentStudentNameQuery = "SELECT student_name FROM education_student_library_borrow_year_2027 WHERE id = '$id'";
    ...
    $query = "UPDATE education_student_library_borrow_year_2027 SET $updateString WHERE id = '$id'";

    // Execute the query
    if (mysqli_query($link, $query)) {
        // Redirect to the main page or display a success message
        header('Location: admin_education_student_library_borrow_year_2027.php'); // Adjust as necessary
        exit;
    } else {
