...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $check_sql = "SELECT student_name FROM guild_math_2028 WHERE student_name = '$student_name'";
    $check_result = mysqli_query($link, $check_sql);

    if (mysqli_num_rows($check_result) > 0) {
        // If student_id exists, show alert and do not proceed with registration
        echo '<script>alert("Student Name already exists. Please use a different Student ID.");</script>';
        echo '<script>window.location.href = "admin_guild_math_year_2028.php";</script>';
    } else {
        // Insert data into the database
        $sql = "INSERT INTO guild_math_2028 (year, student_name, section) 
