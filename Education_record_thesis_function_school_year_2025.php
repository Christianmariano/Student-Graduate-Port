...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $major = mysqli_real_escape_string($link, trim($_POST['major']));
        ...
        $sql = "INSERT INTO education_student_record_school_year_2025 (student_name, major) 
                VALUES ('$student_name', '$major')";

        if (mysqli_query($link, $sql)) {
            echo '<script>alert("Record added successfully.");</script>';
            echo '<script>window.location.href = "admin_education_thesis_school_year_2025.php";</script>';
        } else {
