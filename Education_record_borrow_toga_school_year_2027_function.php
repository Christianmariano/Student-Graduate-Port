...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $major = mysqli_real_escape_string($link, trim($_POST['major']));
        ...
        $sql = "INSERT INTO education_student_record_school_year_2027 (student_id, student_name, toga_amount, payment_status,msjor, rent_status) 
                VALUES ('$student_id', '$student_name', '$toga_amount', '$payment_status', '$rent_status','$major')";

        if (mysqli_query($link, $sql)) {
            echo '<script>alert("Record added successfully.");</script>';
            echo '<script>window.location.href = "admin_cashier_education_department_record_borrow_toga_school_year_2027.php";</script>';
        } else {
